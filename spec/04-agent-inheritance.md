...
│  ─────────────────────────────                          │
│  app_settings        用户的勾选 + 路径 override        │
...
type InheritedEndpoint struct {
    ...
    APIKey       string  // 扫描结果暂存(仅在内存中流转;入库前剥离,见 §6.4)
    ExtrasJSON   string  // 灵活字段: OAuth token / 订阅信息 / 自定义 header(oauth_token 入库前剥离)
}
```
...
- daemon 调 Scanner 出错 → 把 error 写入 `app_settings.last_error`，UI 显示
...
### 7.1 app_settings — 用户的接入选择

```sql
CREATE TABLE app_settings (
    app_id        TEXT PRIMARY KEY,        -- 'openclaw' / 'claude-code' / 'cursor' / ...
...
CREATE TABLE inherited_endpoints (
    ...
    protocol_hint   TEXT,
    extras_json     TEXT,                    -- 灵活字段(凭证字段入库前剥离)
    captured_at     INTEGER NOT NULL,
    PRIMARY KEY (app_id, provider),
    FOREIGN KEY (app_id) REFERENCES app_settings(app_id) ON DELETE CASCADE
);
```

> **凭证不入库(migration 022 起)**:`api_key` 列已删除;`extras_json` 里的
> `oauth_token` 在写库前剥离。扫描出的凭证只存在于守护进程内存中的
> `agentscan.CredStore`(启动同步扫描 + 每分钟周期重扫 + 手动 rescan 即时刷新),
> 路由 keyFn、模型 discovery、MiniMax 配额轮询全部从内存取。
> 升级时 migration 022 先 `json_remove` 再 `DROP COLUMN`,随后 `VACUUM`
> 确保旧 key 字节从 db 文件中物理消失。
...
```json
[
  {"app_id": "openclaw",    "display_name": "OpenClaw",     "default_path": "/Users/frank/.openclaw/openclaw.json"},
  {"app_id": "claude-code", "display_name": "Claude Code",  "default_path": "/Users/frank/.zshrc"},
  ...
]
```
...
返回 `app_settings` 表 + 关联的 inherited_endpoints 计数：
...
### 8.3 POST /internal/apps/{app_id}/rescan

```
body: { "path": "/optional/custom/path" }
```

- 写 `app_settings.config_path = path`（或默认）
- 调 `Scanners[app_id].Scan(path)`
- 成功 → 写 `inherited_endpoints` (replace by app_id)，更新 `app_settings.last_scanned_at`
- 失败 → 更新 `app_settings.last_error`
- 广播 SSE `"agents_changed"`

### 8.4 POST /internal/apps/{app_id}/enable | /disable

切换 `app_settings.enabled`，广播 SSE 让 routing engine 即时刷新候选 endpoint。

### 8.5 DELETE /internal/apps/{app_id}

删除 `app_settings` 那一行 + cascade 删 `inherited_endpoints`。
...
- 写入：`token_price_api` 表（migration 001）
...
- `discoverProviderModels` 从内存 `CredStore` 拿 key(用户在 agent 里配的 key 自动被 daemon 用来 discover,且不落盘)
...
- OAuth token 优先从内存 `agentscan.CredStore` 读(Scanner 抓出后只进内存,不落盘);请求 header 捕获缓存(30min TTL)作为兜底

### 9.4 ✅ model_catalog 表（已删,migration 015）

**当时状态**：表存在但实际**没数据**（LiteLLM ETag 304 跳过写入）。
...
**操作**（已完成,migration 015 / v2.3.12 #60）：`model_catalog` 表与相关代码路径已删除。`token_price_api` 表保留（pricing 数据仍然要）。
...
| `discoverProviderModels` 拿 key 的逻辑 | 用 `settings.ProviderKeys` | ✅ 已改为内存 `CredStore`(凭证不落盘) |
...
```
用户 override (app_settings.config_path)       ← 优先
       ↓ 没改
...
- [x] `discoverProviderModels` 从内存 `CredStore` 取 key(2026-06-10 起凭证不落盘)
