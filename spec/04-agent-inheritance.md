...
**操作**：v2.0.51 或后续版本删除 migration 004 创建的 `model_catalog` 表、`internal/storage/model_catalog.go`、`pricing.OnSync` 推 catalog 到 adapter 的代码路径。`token_price_api` 表保留（pricing 数据仍然要）。
