...
export default function AgentInheritanceSection() {
  ...
  if (loadingSupported) {
    return <SectionShell><p className="text-xs text-gray-400">Loading…</p></SectionShell>
  }
  ...
  return (
    <SectionShell>
      <header className="flex items-baseline justify-between mb-3">
        <h2 className="text-sm font-semibold">Agent Inheritance</h2>
        <span className="text-xs text-gray-400">
          {supported.length} known · {(configured ?? []).filter((c) => c.enabled).length} enabled
        </span>
      </header>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            {!config && (
              <span className="text-[11px] font-medium text-gray-400">Not configured</span>
            )}
          </div>
          <p className="text-[11px] text-gray-400 truncate">
            {inherited > 0
              ? `${inherited} provider${inherited === 1 ? '' : 's'} inherited`
              : enabled ? 'No providers inherited yet' : '—'}
            {lastScannedAt && (
              <>
                {' · last scan '}
        ...
        <div className="flex items-center gap-1">
          ...
          >
            <Power className="w-3.5 h-3.5" /> {enabled ? 'Disable' : 'Enable'}
          </button>
          <button
            type="button"
      ...
      {expanded && (
        <div className="mt-3 ml-10 space-y-2">
          <div className="flex items-center gap-2 text-xs">
            <span className="text-gray-400 w-20 shrink-0">Config path</span>
            {editPath ? (
              <input
                type="text"
                value={pathDraft}
          ...
          <p className="text-[11px] text-gray-400">
            Default: <code>{supported.default_path}</code>
          </p>
        </div>
      )}
    </li>
