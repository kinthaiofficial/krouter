...
//	0:              candidate is not cheaper — no incentive to switch
//	1:              candidate is so cheap that even 100% hit rate can't save
//	                the bound model — switching always wins
//	-1:             prices unknown — caller should treat as "no signal"
func cacheHitBreakeven(boundPrice, candidatePrice float64) float64 {
	if boundPrice <= 0 || candidatePrice <= 0 {
		return -1
	}
