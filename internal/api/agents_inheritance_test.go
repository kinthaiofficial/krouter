...
func TestAgentsSupported_ReturnsScannerRegistry(t *testing.T) {
	...
	srv.Handler().ServeHTTP(w, authedReq(t, http.MethodGet, "/internal/apps/supported", ""))

	require.Equal(t, http.StatusOK, w.Code)
	var got []supportedAppJSON
...
func TestAgentsConfigured_EmptyWhenNoRows(t *testing.T) {
	...
	w := httptest.NewRecorder()
	srv.Handler().ServeHTTP(w, authedReq(t, http.MethodGet, "/internal/apps/configured", ""))
	require.Equal(t, http.StatusOK, w.Code)

	var got []configuredAppJSON
	require.NoError(t, json.Unmarshal(w.Body.Bytes(), &got))
	assert.Empty(t, got)
}
...
func TestAgentRescan_WritesInheritedRows(t *testing.T) {
	...
	srv.Handler().ServeHTTP(w, authedReq(t, http.MethodPost,
		"/internal/apps/openclaw/rescan", `{"path":"/custom/path"}`))
	...
	row, _ := store.GetAppSetting(context.Background(), "openclaw")
...
func TestAgentRescan_UnknownAgent(t *testing.T) {
	...
	w := httptest.NewRecorder()
	srv.Handler().ServeHTTP(w, authedReq(t, http.MethodPost,
		"/internal/apps/unknown-vendor/rescan", `{}`))
...
func TestAgentEnableDisable(t *testing.T) {
	...
	w = httptest.NewRecorder()
	srv.Handler().ServeHTTP(w, authedReq(t, http.MethodPost,
		"/internal/apps/openclaw/disable", ""))
	require.Equal(t, http.StatusOK, w.Code)

	row, _ = store.GetAppSetting(context.Background(), "openclaw")
	assert.False(t, row.Enabled)

	eps, _ := store.ListInheritedEndpointsByApp(context.Background(), "openclaw")
...
func TestAgentDelete_RemovesRow(t *testing.T) {
	...
	w := httptest.NewRecorder()
	srv.Handler().ServeHTTP(w, authedReq(t, http.MethodDelete, "/internal/apps/openclaw", ""))
	require.Equal(t, http.StatusOK, w.Code)

	row, _ := store.GetAppSetting(ctx, "openclaw")
	assert.Nil(t, row, "agent_settings row should be gone")

	eps, _ := store.ListInheritedEndpointsByApp(ctx, "openclaw")
